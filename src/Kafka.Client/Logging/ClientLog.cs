...
namespace Kafka.Client.Logging
{
    ...
    {
        [LoggerMessage(EventId = 1001, Level = LogLevel.Information, Message = "{error}", SkipEnabledCheck = true)]
        internal static partial void LogApiError(this ILogger logger, in ApiError error);
        [LoggerMessage(EventId = 1002, Level = LogLevel.Information, Message = "{header}|{error}", SkipEnabledCheck = true)]
        internal static partial void LogApiError(this ILogger logger, in RequestHeaderData header, in ApiError error);
        [LoggerMessage(EventId = 1003, Level = LogLevel.Warning, Message = "No valid bootstrap servers", SkipEnabledCheck = false)]
        internal static partial void EmptyBootstrapServers(this ILogger logger);
        [LoggerMessage(EventId = 1004, Level = LogLevel.Error, Message = "Invalid bootstrap url: {bootstrapUrl}", SkipEnabledCheck = false)]
