...
namespace Kafka.Client.Clients.Consumer
{
        ...
        IConsumerChannel
    {
        ...
        private async Task CheckState(CancellationToken cancellationToken)
        {
            ...
            {
                var listOffsetsRequest = new ListOffsetsRequest(
                    _nodeId.Value,
                    _isolationLevel,
                    missingOffsets
                        .GroupBy(r => r.Key.Topic)
                        .Select(t => new ListOffsetsRequest.ListOffsetsTopic(
                            t.Key.TopicName,
                            t.Select(p => new ListOffsetsRequest.ListOffsetsTopic.ListOffsetsPartition(
        ...
        private async Task FetchLoop(CancellationToken cancellationToken)
        {
            while (_running && !cancellationToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    var taskCompletionSource = new TaskCompletionSource<IReadOnlyDictionary<TopicPartition, Offset>>(TaskCreationOptions.RunContinuationsAsynchronously);
                    _fetchCallbacks.Add(new(fetchRecords, taskCompletionSource), cancellationToken);
                    var readOffsets = await taskCompletionSource.Task.ConfigureAwait(false);
                    foreach ((var topicPartition, var offset) in readOffsets)
                        _topicPartitionOffsets[topicPartition] = offset + 1;
                }
                catch (OperationCanceledException)
                {
                    _logger.LogInformation("Fetch loop terminated");
                }
                catch(Exception ex)
                {
                    _logger.LogInformation("Fetch loop unexpected error {ex}", ex);
                    break;
                }
            }
        }

        private ImmutableArray<FetchRecords> CheckResult(FetchResponse fetchResponse)
        {
