...
namespace Kafka.Client.Clients.Consumer
{
        ...
        IConsumer<TKey, TValue>
    {
        ...
        async ValueTask<IInputStreamApplication<TKey, TValue>> IConsumer<TKey, TValue>.CreateInstance(
            TopicName topicName,
            CancellationToken cancellationToken
        )
        {
        ...
        )
        {
            ...
            var connection = new TcpConnection(host, port);
        ...
        )
        {
            ...
            var metadataRequest = new MetadataRequest(
                topicNames.Select(t =>
                    new MetadataRequest.MetadataRequestTopic(
                        Guid.Empty,
                        t,
                        ImmutableArray<TaggedField>.Empty
                    )
                ...
                metadataResponse
                    ...
                    .Select(t => new ListOffsetsRequest.ListOffsetsTopic(
                        ...
                        t.PartitionsField
                            ...
                            .Select(p => new ListOffsetsRequest.ListOffsetsTopic.ListOffsetsPartition(
                                p.PartitionIndexField,
                                p.LeaderEpochField,
        ...
        )
        {
            ...
            var offsetFetchRequest = new OffsetFetchRequest(
                Config.GroupId ?? "",
                metadataResponse.TopicsField.Select(t => new OffsetFetchRequest.OffsetFetchRequestTopic(
                    t.NameField ?? "",
                    t.PartitionsField
                        .Where(p => !filter.Any() || filter.Contains(new TopicPartition(t.NameField, p.PartitionIndexField)))
                        .Select(p =>
                ...
                .ToImmutableArray(),
                ImmutableArray.Create(new OffsetFetchRequest.OffsetFetchRequestGroup(
                    Config.GroupId ?? "",
                    metadataResponse.TopicsField.Select(t => new OffsetFetchRequest.OffsetFetchRequestGroup.OffsetFetchRequestTopics(
                        t.NameField ?? "",
                        t.PartitionsField
                            .Where(p => !filter.Any() || filter.Contains(new TopicPartition(t.NameField, p.PartitionIndexField)))
                            .Select(p =>
