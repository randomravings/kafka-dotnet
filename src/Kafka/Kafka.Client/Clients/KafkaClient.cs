...
namespace Kafka.Client.Clients
{
        ...
        where TConfig : notnull, ClientConfig
    {
        ...
        protected async ValueTask<TProtocol> GetControllerProtocol<TProtocol>(            
            Func<IConnection, TConfig, ILogger<TClient>, TProtocol> builder,
        ...
        protected IConnection RandomizeConnection()
        {
...
#pragma warning restore CA5394 // Do not use insecure randomness
        ...
        protected static ImmutableArray<DnsEndPoint> Parse(string bootStrapServer) =>
            bootStrapServer
                .Split(',')
                .Select(r => r.Split(':'))
                .Select(r => new DnsEndPoint(r[0], int.Parse(r[1], NumberStyles.None, CultureInfo.InvariantCulture), System.Net.Sockets.AddressFamily.InterNetwork))
                .ToImmutableArray()
        ;

        protected static IConnection CreateConnection(string host, int port) =>
