...
namespace Kafka.Client.Messages
{
    ...
    {
        private static readonly Func<Stream, BeginQuorumEpochResponse>[] READ_VERSIONS = {
            b => ReadV00(b),
        };
        private static readonly Action<Stream, BeginQuorumEpochResponse>[] WRITE_VERSIONS = {
            (b, m) => WriteV00(b, m),
        };
        public static BeginQuorumEpochResponse Read(Stream buffer, short version) =>
            READ_VERSIONS[version](buffer)
        ;
        public static void Write(Stream buffer, short version, BeginQuorumEpochResponse message) =>
            WRITE_VERSIONS[version](buffer, message)
        ;
        private static BeginQuorumEpochResponse ReadV00(Stream buffer)
        {
        ...
        private static void WriteV00(Stream buffer, BeginQuorumEpochResponse message)
        {
