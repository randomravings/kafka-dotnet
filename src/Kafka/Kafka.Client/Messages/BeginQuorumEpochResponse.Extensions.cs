...
namespace Kafka.Client.Messages
{
   ...
   public static class BeginQuorumEpochResponseSerde
   {
       ...
       public static (int Offset, BeginQuorumEpochResponse Value) Read(byte[] buffer, int index, short version) =>
           READ_VERSIONS[version](buffer, index)
       ;
       public static int Write(byte[] buffer, int index, BeginQuorumEpochResponse message, short version) =>
           WRITE_VERSIONS[version](buffer, index, message)
       ;
       private static (int Offset, BeginQuorumEpochResponse Value) ReadV00(byte[] buffer, int index)
       {
