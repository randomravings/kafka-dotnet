...
namespace Kafka.Client.Messages
{
   ...
   public static class EndQuorumEpochRequestSerde
   {
       ...
       public static (int Offset, EndQuorumEpochRequest Value) Read(byte[] buffer, int index, short version) =>
           READ_VERSIONS[version](buffer, index)
       ;
       public static int Write(byte[] buffer, int index, EndQuorumEpochRequest message, short version) =>
           WRITE_VERSIONS[version](buffer, index, message)
       ;
       private static (int Offset, EndQuorumEpochRequest Value) ReadV00(byte[] buffer, int index)
       {
       ...
       private static class TopicDataSerde
       {
           ...
           private static class PartitionDataSerde
           {
               public static (int Offset, PartitionData Value) ReadV00(byte[] buffer, int index)
               {
                   ...
                   (index, var preferredSuccessorsField) = Decoder.ReadArray<int>(buffer, index, Decoder.ReadInt32);
                   if (preferredSuccessorsField == null)
                       throw new NullReferenceException("Null not allowed for 'PreferredSuccessors'");
                   return (index, new(
                       partitionIndexField,
                       leaderIdField,
                       leaderEpochField,
                       preferredSuccessorsField.Value
                   ));
               }
