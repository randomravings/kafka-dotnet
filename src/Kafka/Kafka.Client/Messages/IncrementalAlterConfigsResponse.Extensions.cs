...
namespace Kafka.Client.Messages
{
   ...
   public static class IncrementalAlterConfigsResponseSerde
   {
       ...
       public static (int Offset, IncrementalAlterConfigsResponse Value) Read(byte[] buffer, int index, short version) =>
           READ_VERSIONS[version](buffer, index)
       ;
       public static int Write(byte[] buffer, int index, IncrementalAlterConfigsResponse message, short version) =>
           WRITE_VERSIONS[version](buffer, index, message)
       ;
       private static (int Offset, IncrementalAlterConfigsResponse Value) ReadV00(byte[] buffer, int index)
       {
       ...
       private static (int Offset, IncrementalAlterConfigsResponse Value) ReadV01(byte[] buffer, int index)
       {
           (index, var throttleTimeMsField) = Decoder.ReadInt32(buffer, index);
           (index, var responsesField) = Decoder.ReadCompactArray<AlterConfigsResourceResponse>(buffer, index, AlterConfigsResourceResponseSerde.ReadV01);
           if (responsesField == null)
               throw new NullReferenceException("Null not allowed for 'Responses'");
